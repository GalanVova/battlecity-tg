...
Bullet.prototype._tankCollision = function (event) {
  ...
  // Враги не стреляют друг в друга
  if (otherTank.isEnemy() && this._tank.isEnemy()) {
    return false;
  }
  // Игроки не убивают друг друга (как в NES оригинале)
  // Пуля игрока просто уничтожается при попадании в союзника
  if (otherTank.isPlayer() && this._tank.isPlayer()) {
    return false;
  }
  return true;
};
